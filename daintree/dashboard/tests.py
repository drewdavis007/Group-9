...
class DashboardViewTests(TestCase):
    def setUp(self):
        ...
        self.item = Item.objects.create(name="Laptop", description="A powerful machine.", price=1000.00, category=self.category, created_by=self.user, is_approved=True)
    ...
    def test_dashboard_displays_items_for_seller(self):
    ...
    def test_return_item(self):
        order = Order.objects.create(user=self.user, paid_amount=1000.00)
        order_item = OrderItem.objects.create(item=self.item, order=order, price=self.item.price, quantity=1)
        ...
        self.assertTrue(order_item.is_returned)
        self.assertRedirects(response, reverse('dashboard:index'))
        self.assertEqual(OrderItem.objects.filter(is_returned=True).count(), 1)
